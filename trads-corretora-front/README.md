...
Todas as interfaces estão centralizadas em `core/interfaces/`, com contratos bem definidos entre front e back. Destaque para `PageResponse<T>`, que é genérica e reflete o padrão `Page<>` do Spring Data, e os DTOs de cada view do dashboard (`VwGainDTO`, `VwLossDTO`, `VwTimeStageDTO`, `VwTransitionsDTO`, `VwLossConversion`).
